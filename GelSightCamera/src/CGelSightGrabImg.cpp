...
int CGelSightGrabImg::SetCameraInitFrame(cv::Mat &outInitBluredImg)
{
	//===================================================================================
    // get initial frame
    ...
    localimg_num_float = 1.0/localimg_num_float;
...
void CGelSightGrabImg::SetSaveDataFolderName(const std::string InSaveDataFolderName)
{
    ...
    // creat the folder
    int status;
    DIR* dir = opendir(InSaveDataFolderName.c_str());
    if (dir){
        std::cout << "SaveDataFolderName = " << InSaveDataFolderName << std::endl;
    }else{
        status = mkdir(InSaveDataFolderName.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
        std::cout << "status = " << status << std::endl;
    }
        
    // with read/write/search permissions for owner and group, and with read/search permissions for others.
    status = mkdir(m_ConstSaveDataFolderName.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
    std::cout << "status = " << status << std::endl;
