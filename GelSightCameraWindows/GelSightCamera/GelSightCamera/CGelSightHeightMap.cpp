...
void CGelSightHeightMap::ShowGradImg(const cv::Mat inGradX, const cv::Mat inGradY)
{
	...
	for (int i = 0; i < localImgRows; i++){
		...
		for (int j = 0; j < localImgCols; j++){
			...
			int local_index_gx = (int)(std::floor((HEIGHTMAP_MAXGRAD_RATIO * std::tan(localGx) + 0.5) * JET_COLORMAP_BINNUM));
			...
			int local_index_gy = (int)(std::floor((HEIGHTMAP_MAXGRAD_RATIO * std::tan(localGy) + 0.5) * JET_COLORMAP_BINNUM));
			...
			int local_index_mag = (int)(std::floor((HEIGHTMAP_MAXGRAD_RATIO * std::tan(localMag) + 0.5) * JET_COLORMAP_BINNUM));
...
void CGelSightHeightMap::ShowHeightMap(const cv::Mat inHeightMap)
{
	...
	for (int i = 0; i < localImgRows; i++){
		...
		for (int j = 0; j < localImgCols; j++){
			...
			int local_color_index = (int)(std::floor((HEIGHTMAP_MAXHEIGHT_RATIO * localValue) * JET_COLORMAP_BINNUM));
		...
		for (int iCol = 0; iCol < ImgWidth; iCol++){
			if((*localMaskImg_ptr) > 0){
				...
				for (int i = 0; i < 3; i++){
					...
					localIntValue[i] = (int)(std::floor(localFloatValue + 0.5));
